#[path = "../../trino/src/config.rs"]
mod config;
#[path = "../../trino/src/decode.rs"]
mod decode;
#[path = "../../trino/src/error.rs"]
mod error;
#[path = "../../trino/src/response.rs"]
mod response;
#[path = "../../trino/src/urlencoded.rs"]
mod urlencoded;
...
// Documents: https://prestodb.io/docs/current/develop/client-protocol.html

const USER: &str = "X-Presto-User";
const SOURCE: &str = "X-Presto-Source";
const TIME_ZONE: &str = "X-Presto-Time-Zone";
const CATALOG: &str = "X-Presto-Catalog";
const SET_CATALOG: &str = "X-Presto-Set-Catalog";
const SCHEMA: &str = "X-Presto-Schema";
const SET_SCHEMA: &str = "X-Presto-Set-Schema";
const TRANSACTION_ID: &str = "X-Presto-Transaction-Id";
const STARTED_TRANSACTION_ID: &str = "X-Presto-Started-Transaction-Id";
const CLEAR_TRANSACTION_ID: &str = "X-Presto-Clear-Transaction-Id";
const ROLE: &str = "X-Presto-Role";
const SET_ROLE: &str = "X-Presto-Set-Role";
const SESSION: &str = "X-Presto-Session";
const SET_SESSION: &str = "X-Presto-Set-Session";
const CLEAR_SESSION: &str = "X-Presto-Clear-Session";
const ADDED_PREPARE: &str = "X-Presto-Added-Prepare";
const PREPARED_STATEMENT: &str = "X-Presto-Prepared-Statement";
const DEALLOCATED_PREPARE: &str = "X-Presto-Deallocated-Prepare";
...
impl Connection {
    pub fn open_with(config: Config) -> Result<Self> {
        ...
        let mut headers = HeaderMap::new();
        {
            ...
            {
                // NOTE:
                // Presto does not support OFFSET by default; enable it via session property.
                // This is Presto-specific — Trino does not need this setting.
                Self::insert_header(
                    &mut headers,
                    SESSION,
                    "offset_clause_enabled=true".to_string(),
                )?;
            }
...
// NOTE: These tests are copied from Trino. Trino and Presto are largely compatible at the protocol level, but there may be differences in some edge cases.
#[cfg(test)]
mod tests {
    use super::*;
    use query::Value;
    ...
    async fn test_headers_set() {
        ...
        let result = conn
            .query("SELECT count(*) FROM runtime.nodes")
            .await
            .unwrap();
        assert!(result.rows.len() > 0);
        conn.query("USE memory.default").await.unwrap();
        let result = conn
            .query("SELECT count(*) FROM system.runtime.nodes")
            .await
            .unwrap();
        assert!(result.rows.len() > 0);
    }
    ...
    async fn test_headers_set_round_trip() {
        ...
        let result = conn.query("SELECT count(*) FROM nodes").await.unwrap();
        assert!(result.rows.len() > 0);
    }
    ...
    async fn test_headers_set_schema_update() {
        ...
        let result = conn
            .query("SELECT table_name FROM tables LIMIT 1")
            .await
            .unwrap();
        assert!(result.rows.len() > 0);
    }
        ...
        let result = conn
            .query(
                "SELECT
                    ...
                    CAST(-9223372036854775807 - 1 AS BIGINT) AS bigint_min,
