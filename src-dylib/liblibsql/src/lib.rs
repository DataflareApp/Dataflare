...
use libsql::{Builder, Connection, OpenFlags, Value as LibSqlValue};
...
extern "C" fn df_execute(conn: *mut Connection, sql: StringRef, error: *mut ErrorMessage) {
    let call = || {
        ...
        RUNTIME
            .block_on(conn.execute(sql.as_str(), ()))
            .string_err()?;
        Ok(())
    };
    if let Err(err) = call() {
...
extern "C" fn df_execute_batch(conn: *mut Connection, sql: StringRef, error: *mut ErrorMessage) {
    let call = || {
        let conn = unsafe { &*conn };
        RUNTIME
            .block_on(conn.execute_batch(sql.as_str()))
            .string_err()?;
        Ok(())
    };
    if let Err(err) = call() {
...
extern "C" fn df_query(
    ...
    fn to_value(val: LibSqlValue) -> QueryValue {
        match val {
            LibSqlValue::Null => QueryValue::Null,
            LibSqlValue::Integer(v) => QueryValue::I64(v),
            LibSqlValue::Real(v) => QueryValue::F64(v),
            LibSqlValue::Text(v) => QueryValue::String(v),
            LibSqlValue::Blob(v) => QueryValue::Bytes(v),
        }
    }
        ...
        RUNTIME
            .block_on(async {
                ...
                let columns = stmt
                    ...
                    .map(|col| QueryColumn {
                        name: col.name().into(),
                        datatype: col.decl_type().unwrap_or("").into(),
                    })
                    .collect::<Vec<_>>();
                ...
                while let Some(row) = rows.next().await? {
                    ...
                    for i in 0..columns.len() {
                        let v = row.get_value(i as i32)?;
                        vec.push(to_value(v));
                    }
                    values.push(vec);
                }

                Ok::<Query, libsql::Error>(Query {
                    columns,
                    rows: values,
...
mod tests {
    ...
    fn test_connect_error() {
        ...
        assert_eq!(
            error.as_str(),
            "Failed to connect to database: `Unable to open connection to local database /a/b/c/: 14`"
        );
        df_free_error(error);
    }
    ...
    fn test_transaction_fail() {
        ...
        assert_eq!(
            error.as_str(),
            r#"SQLite failure: `near "error": syntax error`"#,
        );
        df_free_error(error);
        let query = df_query(
            conn,
